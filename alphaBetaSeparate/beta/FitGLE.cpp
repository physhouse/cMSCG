...
void FitGLE::accumulateNormalEquation()
{
    ...
    for (int i=0; i<nall; i++)
    {
        for (int j = i+1; j<nall; j++)
        {
            ...
            double max = std::max(std::max(ricjc, riojc), std::max(ricjo, rjojc));
            if (max < info->end && max > info->start)
            {
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     ...
                     frameMatrix[m][3*i]     += phim * ecom[0] / beta;
                     frameMatrix[m][3*i + 1] += phim * ecom[1] / beta;
                     frameMatrix[m][3*i + 2] += phim * ecom[2] / beta;
                     frameMatrix[m][3*j]     -= phim * ecom[0] / beta;
                     frameMatrix[m][3*j + 1] -= phim * ecom[1] / beta;
                     frameMatrix[m][3*j + 2] -= phim * ecom[2] / beta;
                }
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     // For all three dimensions
                     frameMatrix[nSplines + m][3*i]     += phim * ricjc * ecom[0] / kco / beta;
                     frameMatrix[nSplines + m][3*i + 1] += phim * ricjc * ecom[1] / kco / beta;
                     frameMatrix[nSplines + m][3*i + 2] += phim * ricjc * ecom[2] / kco / beta;
                     frameMatrix[nSplines + m][3*j]     -= phim * ricjc * ecom[0] / kco / beta;
                     frameMatrix[nSplines + m][3*j + 1] -= phim * ricjc * ecom[1] / kco / beta;
                     frameMatrix[nSplines + m][3*j + 2] -= phim * ricjc * ecom[2] / kco / beta;
                }
            }  
        }
    }
