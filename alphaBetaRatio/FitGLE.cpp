...
void FitGLE::accumulateNormalEquation()
{
    ...
    for (int i=0; i<nall; i++)
    {
        for (int j = i+1; j<nall; j++)
        {
            ...
            double max = std::max(std::max(riojo, riojc), std::max(ricjo, std::max(rjojc, ricjc)));
            if (max < info->end && max > info->start)
            {
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     ...
                     frameMatrix[m][3*nall + 3*i]     += beta * phim * eiojo[0] / alpha;
                     frameMatrix[m][3*nall + 3*i + 1] += beta * phim * eiojo[1] / alpha;
                     frameMatrix[m][3*nall + 3*i + 2] += beta * phim * eiojo[2] / alpha;
                     frameMatrix[m][3*nall + 3*j]     -= beta * phim * eiojo[0] / alpha;
                     frameMatrix[m][3*nall + 3*j + 1] -= beta * phim * eiojo[1] / alpha;
                     frameMatrix[m][3*nall + 3*j + 2] -= beta * phim * eiojo[2] / alpha;
                }
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     ...
                     frameMatrix[nSplines + m][3*nall + 3*i]     += phim * riojo * eiojo[0] * beta / koc / alpha;
                     frameMatrix[nSplines + m][3*nall + 3*i + 1] += phim * riojo * eiojo[1] * beta / koc / alpha;
                     frameMatrix[nSplines + m][3*nall + 3*i + 2] += phim * riojo * eiojo[2] * beta / koc / alpha;
                     frameMatrix[nSplines + m][3*nall + 3*j]     -= phim * riojo * eiojo[0] * beta / koc / alpha;
                     frameMatrix[nSplines + m][3*nall + 3*j + 1] -= phim * riojo * eiojo[1] * beta / koc / alpha;
                     frameMatrix[nSplines + m][3*nall + 3*j + 2] -= phim * riojo * eiojo[2] * beta / koc / alpha;
                }
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     // For all three dimensions
                     frameMatrix[nSplines + m][3*i]     += phim * ricjc * eicjc[0] / kco;
                     frameMatrix[nSplines + m][3*i + 1] += phim * ricjc * eicjc[1] / kco;
                     frameMatrix[nSplines + m][3*i + 2] += phim * ricjc * eicjc[2] / kco;
                     frameMatrix[nSplines + m][3*j]     -= phim * ricjc * eicjc[0] / kco;
                     frameMatrix[nSplines + m][3*j + 1] -= phim * ricjc * eicjc[1] / kco;
                     frameMatrix[nSplines + m][3*j + 2] -= phim * ricjc * eicjc[2] / kco;

                     frameMatrix[nSplines + m][3*nall + 3*i]     += phim * ricjc * eicjc[0] * alpha / kco / beta;
                     frameMatrix[nSplines + m][3*nall + 3*i + 1] += phim * ricjc * eicjc[1] * alpha / kco / beta;
                     frameMatrix[nSplines + m][3*nall + 3*i + 2] += phim * ricjc * eicjc[2] * alpha / kco / beta;
                     frameMatrix[nSplines + m][3*nall + 3*j]     -= phim * ricjc * eicjc[0] * alpha / kco / beta;
                     frameMatrix[nSplines + m][3*nall + 3*j + 1] -= phim * ricjc * eicjc[1] * alpha / kco / beta;
                     frameMatrix[nSplines + m][3*nall + 3*j + 2] -= phim * ricjc * eicjc[2] * alpha / kco / beta;
                }
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     // For all three dimensions
                     frameMatrix[2*nSplines + m][3*i]     += phim * eicjc[0];
                     frameMatrix[2*nSplines + m][3*i + 1] += phim * eicjc[1];
                     frameMatrix[2*nSplines + m][3*i + 2] += phim * eicjc[2];
                     frameMatrix[2*nSplines + m][3*j]     -= phim * eicjc[0];
                     frameMatrix[2*nSplines + m][3*j + 1] -= phim * eicjc[1];
                     frameMatrix[2*nSplines + m][3*j + 2] -= phim * eicjc[2];

                     frameMatrix[2*nSplines + m][3*nall + 3*i]     += phim * eicjc[0] * alpha / beta;
                     frameMatrix[2*nSplines + m][3*nall + 3*i + 1] += phim * eicjc[1] * alpha / beta;
                     frameMatrix[2*nSplines + m][3*nall + 3*i + 2] += phim * eicjc[2] * alpha / beta;
                     frameMatrix[2*nSplines + m][3*nall + 3*j]     -= phim * eicjc[0] * alpha / beta;
                     frameMatrix[2*nSplines + m][3*nall + 3*j + 1] -= phim * eicjc[1] * alpha / beta;
                     frameMatrix[2*nSplines + m][3*nall + 3*j + 2] -= phim * eicjc[2] * alpha / beta;
                }
            }  
        }
    }
    ...
    for (int m=0; m<3*nSplines; m++)
    {
        ...
        for (int k=0; k<3 * nall; k++)
            sum_b += frameMatrix[m][3*nall + k] * (trajFrame->residualForces[2*(k/3)][k%3] + trajFrame->residualForces[2*(k/3)+1][k%3]);
