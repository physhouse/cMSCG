...
void FitGLE::accumulateNormalEquation()
{
    ...
    std::vector<std::vector<double> > frameMatrix(3*nSplines, std::vector<double>(3*nall));
    std::vector<double> frameVector(3*nall);
    ...
    for (int i=0; i<nall; i++)
    {
        for (int j = i+1; j<nall; j++)
        {
            ...
            if (max < info->end && max > info->start)
            {
                ...
                frameVector[3*i]     += beta * table[index][1] * eicjc[0];
                frameVector[3*i + 1] += beta * table[index][1] * eicjc[1];
                frameVector[3*i + 2] += beta * table[index][1] * eicjc[2];
                frameVector[3*j]     -= beta * table[index][1] * eicjc[0];
                frameVector[3*j + 1] -= beta * table[index][1] * eicjc[1];
                frameVector[3*j + 2] -= beta * table[index][1] * eicjc[2];
